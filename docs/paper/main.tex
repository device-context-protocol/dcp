...
calls \texttt{reboot()}. DCP catches the 15~\texttt{reboot} calls via
capability plus a handful of label payloads that arrived with an empty
required field (30\,\% total); OpenAPI catches the same 15~reboots
plus 15~more \texttt{set\_label} payloads via its \texttt{pattern} and
\texttt{maxLength} constraints, for 50\,\% total. The gap on
...
protocols have no such concept and let all of them through. OpenAPI
beats DCP on \emph{prompt injection} because its JSON-Schema
\texttt{pattern} catches injection payloads in free-form string fields
--- a constraint DCP's \texttt{Param} type does not yet expose.
