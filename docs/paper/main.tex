...
and an empirical evaluation in which 295~tool calls produced by two
LLMs (DeepSeek~V3, Qwen2.5-72B-Instruct) in response to six categories
of adversarial prompts are run through the host-side validators of
DCP, Raw MCP, IoT-MCP, and a well-formed OpenAPI~3 schema; DCP
rejects 100\,\% of capability-escalation attempts and 50\,\% of
prompt-injection attempts versus 0--5\,\% for MCP-family protocols,
matching OpenAPI's expressiveness at three orders of magnitude less
firmware footprint. We position DCP as the missing layer between MCP
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item LLM-safety results beyond two LLMs and six attack categories.
        ...
        emitted no parseable OpenAI-style tool calls and was excluded).
        Broader coverage across model families and a larger,
        independently-sourced attack taxonomy is left to follow-up work.
\end{itemize}
