...
those tools to DeepSeek~V3 via the OpenAI-style function-calling API,
and ran 180 adversarial prompts spanning six attack categories. The LLM
emitted 140 parseable tool calls; the remaining 40 prompts resulted in
the LLM either refusing or replying in natural language and are
excluded as having no protocol-layer event to measure. The 140 captured
calls --- real LLM behavior, not hand-constructed inputs --- are then
...
would. Three findings stand out. (i)~The LLM \emph{self-corrects} most
obvious numeric mistakes: prompts like ``set it to 200'' or ``set it to
...
both at 23\,\% / 7\,\%) because the LLM has already absorbed most of
the work. (ii)~Capability escalation is where the protocol layer
genuinely earns its keep: prompts asking to ``reboot the lamp'' caused
the LLM to call \texttt{reboot()} in 29~of 30~cases, and DCP's
mandatory per-intent capability and OpenAPI's OAuth2 scopes both reject
all~29; MCP-family protocols have no caller-capability concept at the
schema layer and let every one through. (iii)~Prompt injection elicits
two distinct LLM behaviors: in 27~of 30~cases the LLM ``absorbs'' the
injection by stuffing the payload into the \texttt{set\_label} text
field, and in the remaining 3~cases it follows the injection and calls
\texttt{reboot()}. DCP catches the 3~\texttt{reboot} calls via capability
(10\,\% in the chart); OpenAPI catches roughly half of the
\texttt{set\_label} payloads via its \texttt{pattern} and
\texttt{maxLength} constraints (50\,\% total). DCP's current
\texttt{Param} type does not yet expose a string \texttt{pattern} field;
adding one is on the v0.4 roadmap and would close the remaining gap.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{hallucination.pdf}
\caption{Schema-layer rejection rates over an empirical corpus of 140
tool calls produced by DeepSeek~V3 in response to adversarial prompts
across six categories. Bars are exact percentages over the calls the
LLM actually emitted; prompts that did not elicit a tool call are
excluded. DCP and OpenAPI both reject every \emph{capability
escalation} attempt because both express per-operation capabilities at
the schema layer; MCP-family protocols have no such concept and let
all of them through. OpenAPI beats DCP on \emph{prompt injection}
because its JSON-Schema \texttt{pattern} catches injection payloads in
free-form string fields --- a constraint DCP's \texttt{Param} type does
not yet expose. Reproduce with
\texttt{python tools/gen\_llm\_corpus.py 3} followed by
\texttt{python tools/bench\_hallucination\_empirical.py}.}
\label{fig:halluc}
\end{figure}
