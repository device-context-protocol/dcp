...
48\,\% of prompt-injection attempts versus 0--6\,\% for MCP-family
protocols, matching OpenAPI's expressiveness at three orders of
magnitude less firmware footprint. We position DCP as the missing layer between MCP
...
100\,\% of capability escalation attempts and 48\,\% of prompt
injection attempts before any byte leaves the host --- versus 0--6\,\%
for Raw MCP and IoT-MCP, and tied with a well-formed OpenAPI\,3 spec
at $\sim$\,1/1000 the runtime footprint OpenAPI requires. (ii) An apples-to-apples wire latency A/B
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  \item An \emph{empirical study} (\S\ref{sec:safety}) in which 660 tool
        calls produced by five real LLMs across four vendors (DeepSeek,
        Alibaba Qwen, Zhipu GLM, MiniMax; all via SiliconFlow) in
        response to adversarial prompts across six attack categories
        are run through each protocol's host-side validator. DCP
        rejects 100\,\% of capability escalation attempts and 48\,\% of
        prompt injection attempts, vs 0--6\,\% for MCP/IoT-MCP. The
        study also produced an end-to-end latency A/B against an
...
660~parseable tool calls (143~DeepSeek, 162~Qwen 2.5, 121~Qwen 3.5,
148~MiniMax, 86~GLM); a sixth candidate, Qwen 2.5-7B-Instruct,
...
no protocol-layer event to measure. The 660~captured calls --- real
...
two distinct LLM behaviors: in the majority of cases the LLM
``absorbs'' the injection by stuffing the payload into the
\texttt{set\_label} text field; in a minority it follows the injection
and calls \texttt{reboot()}. The latter is caught by capability; the
former needs a \emph{string} constraint to reject. The first run of
...
660~tool calls produced by five LLMs across four vendors (DeepSeek\,V3;
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item LLM-safety results beyond five LLMs and six attack categories.
        Our corpus is 660~tool calls from DeepSeek\,V3, Alibaba Qwen
        2.5-72B / Qwen 3.5-35B-A3B, Zhipu GLM-4-32B, and MiniMax M2.5
        (a sixth candidate, Qwen 2.5-7B-Instruct, emitted no parseable
        OpenAI-style tool calls and was excluded). Closed-source
        frontier models (GPT-4o, Claude, Gemini) and an
        independently-sourced attack taxonomy (e.g.\ via AgentDojo) are
...
Across 660 real LLM-emitted tool calls from five models spanning four
vendor families, DCP's structural checks \emph{catch every
capability-escalation attempt} (100\,\% vs 0\,\% for the only
protocols that target comparable hardware) and add no
